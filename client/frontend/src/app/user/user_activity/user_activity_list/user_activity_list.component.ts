...
import {UserPostService, IPost} from "../../user_article/user_article.service";
...
@Component({
  templateUrl: './user_activity_list.component.html',
})
export class UserActivityComponent implements OnInit,AfterViewChecked {
  ...
  ngAfterViewChecked() {
    this.seoService.setTitle(this.translate.instant('enterprise.activity_list'), this.seoService.getTitleContent());
  }
  ...
  ngOnInit() {
    this.articleStatus = [
      {
        id: 1,
        name: 'enterprise.activity_published'
      },
      {
        id: 2,
        name: 'enterprise.offline_activity'
      }
    ];
    this.articleTab(1);
  }
  ...
  async readCategory(): Promise<any> {
    try {
      ...
      for (let item of categories.result[1].children) {
