...
@Component({
  templateUrl: './user_qa_list.component.html',
})
export class UserQaComponent implements OnInit, AfterViewChecked {
  ...
  ngAfterViewChecked(){
    this.seoService.setTitle(this.translate.instant('enterprise.QA_list'),this.seoService.getTitleContent());
  }
  ...
  ngOnInit() {
    this.articleStatus = [
      {
        id: 1,
        name: 'enterprise.qa_published'
      },
      {
        id: 2,
        name: 'enterprise.offline_qa'
      }
    ];
    this.articleTab(1);
  }
  ...
  async readCategory(): Promise<any> {
    try {
      ...
      for (let item of categories.result[2].children) {
